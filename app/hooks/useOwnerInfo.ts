...
import { API_ENDPOINTS, ROUTES } from '../components/general';
...
export const useOwnerInfo = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await fetch(API_ENDPOINTS.MY_ADDRESSES, {
                    method: 'GET',
                    headers: {
    ...
    const submitForm = async (data: FormData) => {
        ...
        try {
            const response = await fetch(API_ENDPOINTS.ORDER_COMPLETION, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Cookie': 'connect.sid=s%3AMSHqW055L1x6v_yZvHrCnfEtiPCDqOk2.Vi%2FeqDS4pcwV05BaghaNfkvKdGB9OcMMBG00LxY9QQ8',
